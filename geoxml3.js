    ...
    // Infowindow-opening event handler
        ...
        if (this.opts.sidebarid) {
            ...
            if(this.sidebarsnippet){
                if (desc2 === '') {
                    desc2 = GeoXml.stripHTML(desc).substring(0,40);
                }
            }
            else {
                desc2 = '';
...
GeoXml.prototype.createPolygon = function(lines,color,width,opacity,fillcolor,fillOpacity, pbounds, name, desc, snippet, folderid, visible,fill,outline,kml_id) {
  var thismap = this.map;
  
  var p = {};    
  p.obj = {"description":desc,"name":name,"snip":snippet };
  p.obj.polylines = []; 
  p.obj.id = kml_id;
...
GeoXml.addSidebar = function(myvar, name, type, e, graphic, ckd, i, snippet) {
   ...
   if(ckd=="false"){ check = ""; }
    ...
    //console.log(name);
   switch(type) {
   case  "marker" :  h = '<li id="'+mid+'" onmouseout="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseout\');" onmouseover="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseover\');" ><input id="'+myvar+''+e+'CB" type="checkbox" style="vertical-align:middle" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'click\');return false;">'+ graphic + '&nbsp;' + name + snippet + '</a>'+'</li>';
   break;
  case  "polyline" :  h = '<li id="'+mid+'"  onmouseout="'+myvar+ '.overlayman.markers['+e+'].onOut();" onmouseover="'+myvar+ '.overlayman.markers['+e+'].onOver();" ><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide(null,this.checked,'+i+')"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'click\');return false;">&nbsp;' + graphic + '&nbsp;' + name + snippet +'</a></span>'+'</li>';
  break;
  case "polygon": h = '<li id="'+mid+'"  onmouseout="'+myvar+ '.overlayman.markers['+e+'].onOut();" onmouseover="'+myvar+ '.overlayman.markers['+e+'].onOver();" ><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'click\');return false;">&nbsp;' + graphic + '&nbsp;' + name + snippet + '</a></span></nobr>'+'</li>';
  break;
 case "groundoverlay": h = '<li id="'+mid+'"><input id="'+myvar+''+e+'CB" type="checkbox" '+check+' onclick="'+myvar+'.showHide('+e+',this.checked)"><span style="margin-top:6px;"><a href="#" onclick="google.maps.event.trigger(' + myvar+ '.overlayman.markers['+e+'],\'zoomto\');return false;">&nbsp;' + graphic + '&nbsp;' + name + snippet + '</a></span>'+'</li>';
   break;
...
GeoXml.prototype.handlePlacemarkGeometry = function(mark, geom, idx, depth, fullstyle) {
    ...
    //    alert(coordset[0].parentNode.nodeName);
        if (nn.match(/^(LinearRing)/i) || nn.match(/^(gml:LinearRing)/i)) {
            ...
            if (name == desc) { desc = ""; }
            if (!!that.opts.createpolygon) {
                that.opts.createpolygon(lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, snippet, idx, visible, fill, outline, kml_id);
            } else {
                that.createPolygon(lines, color, width, opacity, fillcolor, fillOpacity, pbounds, name, desc, snippet, idx, visible, fill, outline, kml_id);
            }
        }
    };
...
GeoXml.prototype.createFolder = function(idx, title, sbid, icon, desc, snippet, keepopen, visible, suppressIt){           
        ...
        if (this.suppressFolders == true || suppressIt){
            ...
            // htm += '<a href="#" onclick="'+this.myvar+'.overlayman.zoomToFolder('+idx+');return false;">' + title + '</a><br>';
